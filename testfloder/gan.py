...
G_opt = tf.train.AdamOptimizer().minimize(G_objective)
D_opt = tf.train.AdamOptimizer().minimize(D_objective)
...
with tf.Session() as sess:
    ...
    for i in range(epochs):
        ...
        if i % 100 == 0:
            loss_G = sess.run(G_opt, feed_dict={
                              z: generate_z(batch_size)})       
            loss_D = sess.run(D_opt, feed_dict={
                X: images[np.random.choice(range(len(images)), batch_size)].reshape(batch_size, x_size),
                z: generate_z(batch_size),
                })
            print('epoch : {} \n G_loss : {} \n D_loss :{}'.format(i,
